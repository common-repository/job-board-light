<?php
global $wpdb;			
	$email_body_cilent = get_option( 'jobboard_order_client_email');
	$order_email_subject_client = get_option( 'jobboard_order_client_email_sub');			
	...
	$dis_amount =get_user_meta($userId, 'jobboard_discount',true);
	
	$package_amount_2=$jobboard_package_cost.' '.$api_currency;
	$discount_2= $dis_amount.' '.$api_currency;
	$total_g =($jobboard_package_cost - $dis_amount).' '.$api_currency;
	...
	// For Admin Email*****	
	$email_body_admin = get_option( 'jobboard_order_admin_email');
	$order_email_subject_admin = get_option( 'jobboard_order_admin_email_sub');			
	...
	$trial_enable= get_post_meta( $package_id,'jobboard_package_enable_trial_period',true); 
	if( $trial_enable!='yes' ){	
