<?php
	...
	$payment_gateway=get_user_meta($userId, 'jobboard_payment_gateway', true);
	if($payment_gateway=='paypal-express'){
		...
		$payment_status=get_user_meta($userId, 'jobboard_payment_status', true);
		if($payment_status=='pending'){
			...
			if(isset($recurringCheck['STATUS'])){
				if($recurringCheck['STATUS']=='Active'){
					...
					$role_package= get_post_meta( $package_id,'jobboard_package_user_role',true); 	
					update_user_meta($userId, 'jobboard_payment_status', 'success');
					$user = new WP_User( $userId );
					$user->set_role($role_package);									
				}
			}
		}
		$exprie_date= strtotime (get_user_meta($userId, 'jobboard_exprie_date', true));	
		...
		if($exprie_date < $current_date){
			...
			if($PROFILEID==''){
				if($exprie_date!=''){
					update_user_meta($userId, 'jobboard_payment_status', 'pending');
					$user = new WP_User( $userId );
					$user->set_role('basic');
				}	
			}
			...
			if(isset($recurringCheck['STATUS'])){
				if($recurringCheck['STATUS']=='Active'){
					...
					// Change exprie_date 
					$package_id=get_user_meta($userId,'jobboard_package_id',true); 
					$jobboard_exprie_date_old =get_user_meta($userId,'jobboard_exprie_date',true);
					$recurring_cycle_count= get_post_meta($package_id,'jobboard_package_recurring_cycle_count',true);
					if($recurring_cycle_count=="" or $recurring_cycle_count==0){$recurring_cycle_count=1;}
					$recurring_cycle_type= get_post_meta($package_id,'jobboard_package_recurring_cycle_type',true);
					$periodNum='';
					...
					update_user_meta($userId, 'jobboard_exprie_date', $new_exp_date); 
					// End exprie_date 
					...
					if(get_post_meta($package_id,'jobboard_package_recurring',true)=='on'){
						$jobboard_package_cost =get_post_meta($package_id,'jobboard_package_recurring_cost_initial',true);
						}else{
						$jobboard_package_cost =get_post_meta($package_id,'jobboard_package_cost',true);
					}
					$api_currency= get_option('epjbjobboard_api_currency' );		
					$total_g = $jobboard_package_cost.' '.$api_currency;												
	...
	if($payment_gateway=='stripe'){
		...
		if($payment_status=='pending'){	
			...
			$cust_id = get_user_meta($userId,'jobboard_stripe_cust_id',true);
			$sub_id = get_user_meta($userId,'jobboard_stripe_subscrip_id',true);
			if($sub_id!=''){							
				try{
