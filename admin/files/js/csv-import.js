...
function update_csv_file(){ 
	   var ajaxurl = dirpro_data.ajaxurl;		  	  
	   var search_params={
		 "action"  :  "finalerp_csv_product_upload",		
...
function save_csv_file_to_database(){
	
	upload_status_checker();
	...
	jQuery("#ep3").show();
	var search_params2={
		"action"  :  	"save_csv_file_to_database",	
...
function upload_status_checker(){
	var importTimer;		
	...
	importTimer = setInterval(function() {
		// Get the current status of the update
			...
			var search_params3={
					"action"  :  "eppro_get_import_status",					
			};
			jQuery.ajax({
				 url : ajaxurl,
				 dataType : "json",
...
function upload_csv_file(update_div_id){
