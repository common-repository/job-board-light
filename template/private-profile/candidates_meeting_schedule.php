<div class="list">
	<?php
		...
		$candidate_schedule= array(
		'relation' => 'AND',
		array(
		'key'     => 'candidate_schedule',
		'value'   => 'yes',
		'compare' => '='
		),
		);
		$candidate_reject= array(
		'relation' => 'OR',
		array(
		...
		$candidate_schedule,$candidate_reject
		);
		?>
			<table id="candidate-meeting" class="table tbl-epmplyer-bookmark" >
			<thead>
				<tr class="">
		...
		if ( $the_query->have_posts() ) {?>
			...
			?>
			<tr id="meeting_<?php echo esc_attr($candidate_post_id);?>" class="">
										...
										<div class="row mt-3">
											<div class="col-md-12">
												...
												<div class="group-button mt-3">
													...
													?>
													<span id="shortlistmeeting<?php echo esc_attr($candidate_post_id); ?>">
														<?php
															if( get_post_meta($candidate_post_id, 'candidate_shortlisted', true)=='yes'){
															?>
															<button id="" class="btn btn-light <?php echo (get_post_meta($candidate_post_id, 'candidate_shortlisted', true)=='yes'?' shortlisted':''); ?>" data-toggle="tooltip"  onclick="candidate_shortlisted_delete('<?php echo esc_attr($candidate_post_id);?>','meeting')" title="<?php esc_html_e( 'Short Listed', 'jobboard' );?>" ><i class="fas fa-user-check"></i></button>
															<?php
																}else{
															?>
															<button id="" class="btn btn-light <?php echo (get_post_meta($candidate_post_id, 'candidate_shortlisted', true)=='yes'?' shortlisted':''); ?>" data-toggle="tooltip"  onclick="candidate_shortlisted('<?php echo esc_html($candidate_post_id);?>','meeting')" title="<?php esc_html_e( 'Make Short Listed', 'jobboard' );?>" ><i class="fas fa-user-check"></i></button>
															<?php
															}
														?>
													</span>
													...
													<span id="rejectmeeting<?php echo esc_attr($candidate_post_id); ?>">
														<?php
															if( get_post_meta($candidate_post_id, 'candidate_reject', true)=='yes'){
															?>
															<button class="btn btn-light btn-delete  shortlisted" onclick="candidate_reject_delete('<?php echo esc_attr($candidate_post_id);?>','meeting')" data-toggle="tooltip" title="<?php esc_html_e( 'Undo Reject', 'jobboard' );?>"><i class="fas fa-user-times"></i></button>
															<?php
																}else{
															?>
															<button class="btn btn-light btn-delete " onclick="candidate_reject('<?php echo esc_attr($candidate_post_id);?>','meeting')" data-toggle="tooltip" title="<?php esc_attr_e( 'Make Reject', 'jobboard' );?>"><i class="fas fa-user-times"></i></button>
															<?php
															}
														?>
													</span>											
