...
<?php
	...
	?>
	<table id="candidatedeletedtable" class="table tbl-epmplyer-bookmark" >
		<thead>
			<tr class="">
	...
	if ( $the_query->have_posts() ) {?>
		...
		?>
		<tr id="deleted_<?php echo esc_html($candidate_post_id);?>" class="">
									...
									<div class="col-md-3">
										<div class="action_dropdown">
											...
											<ul>
												...
												<li>
													<a  onclick="candidate_reject_delete('<?php echo esc_attr($candidate_post_id);?>','list')">
													<i class="fas fa-user-times"></i>
													<?php esc_html_e( 'Undo Reject', 'jobboard' );?>
													</a>
												</li>
												<li>
										...
										<div class="col-md-12">
											...
											<div class="meta-job">
												<span class="location">
													...
													<?php
														if($the_query->post->post_author < 1){
															echo  esc_html(get_post_meta($candidate_post_id, 'phone', true));
															}else{
											...
											<div class="group-button mt-3">
												...
												<span id="shortlistdeleted<?php echo esc_attr($candidate_post_id); ?>">
												<?php
													if( get_post_meta($candidate_post_id, 'candidate_shortlisted', true)=='yes'){
													?>
													<button id="" class="btn btn-light <?php echo (get_post_meta($candidate_post_id, 'candidate_shortlisted', true)=='yes'?' shortlisted':''); ?>" data-toggle="tooltip"  onclick="candidate_shortlisted_delete('<?php echo esc_attr($candidate_post_id);?>','deleted')" title="<?php esc_html_e( 'Short Listed', 'jobboard' );?>" ><i class="fas fa-user-check"></i></button>
													<?php
													}else{
													?>
														<button id="" class="btn btn-light <?php echo (get_post_meta($candidate_post_id, 'candidate_shortlisted', true)=='yes'?' shortlisted':''); ?>" data-toggle="tooltip"  onclick="candidate_shortlisted('<?php echo esc_attr($candidate_post_id);?>','deleted')" title="<?php esc_html_e( 'Make Short Listed', 'jobboard' );?>" ><i class="fas fa-user-check"></i></button>
													<?php
													}
													?>
												</span>
												...
												<span id="rejectlist<?php echo esc_attr($candidate_post_id); ?>">
												<?php
													if( get_post_meta($candidate_post_id, 'candidate_reject', true)=='yes'){
													?>
													<button class="btn btn-light btn-delete  shortlisted" onclick="candidate_reject_delete('<?php echo esc_attr($candidate_post_id);?>','list')" data-toggle="tooltip" title="<?php esc_html_e( 'Undo Reject', 'jobboard' );?>"><i class="fas fa-user-times"></i></button>
													<?php
													}else{
													?>
														<button class="btn btn-light btn-delete " onclick="candidate_reject('<?php echo esc_attr($candidate_post_id);?>','list')" data-toggle="tooltip" title="<?php esc_html_e( 'Make Reject', 'jobboard' );?>"><i class="fas fa-user-times"></i></button>
													<?php
													}
													?>
												</span>	
