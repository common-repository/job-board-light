												...
												foreach ( $categories as $term ) {
													...
													<li>
														<div class="custom-control custom-checkbox">
															<input class="custom-control-input" type="checkbox" name="category_input[]" value="<?php echo esc_attr($term->name);?>" id="<?php echo esc_attr($term->name);?>" <?php echo esc_attr($selected); ?>>
						...
						<div class="col-md-8">
							<div class="content-main-right list-jobs">
								<div class="header-list-job d-flex flex-wrap justify-content-between align-items-center">
									<h4><?php echo esc_html($total_users); ?> <?php esc_html_e('Employers Found', 'jobboard'); ?> </h4>
								</div>
								<div class="list">
									<?php				    
										// User Loop
													...
													<div class="col-md-10">
														<div class="row">
															...
															<div class="meta-job col-md-8">
																<p><i class="fa fa-check-circle"></i><?php esc_html_e('Member Since', 'jobboard'); ?> 
																...
																<?php
																	...
																	?>
																	<p class="cate"><i class="far fa-file"></i> <?php echo esc_html(get_user_meta($user->ID,'company_type',true)); ?></p>
																	<?php
																	}
																?>
															</div>
															<?php
																$total_jobs= $main_class->jobboard_total_job_count($user->ID, $allusers='no' );
																if($total_jobs>0){
																?>
																<div class="number-position col-md-4 align-self-center px-0">
																	<a href="<?php echo get_post_type_archive_link( $directory_url ).'?employer='.esc_attr($user->ID); ?>"><span class="btn btn-light"> <?php echo esc_html($total_jobs);?> <?php esc_html_e('Open Jobs', 'jobboard'); ?></span></a>
																</div>
																<?php
																}
															?>
